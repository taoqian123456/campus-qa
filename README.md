...
浏览器打开 **http://127.0.0.1:8000** → 注册账号 → 管理后台上传政策文档 → 重建索引 → 开始提问。

> 👤 **注册管理员（网页即可，无需命令行）**：在 `.env` 里设好 `ADMIN_INVITE_CODE=你的密令` 并完成初始化后，
> 打开注册页在「管理员密令（选填）」输入框填入该密令，注册成功后即为**管理员**，可直接使用管理后台；
> 不填密令则注册为普通用户。首个**超级管理员**建议仍用 `python create_admin.py` 创建
> （可改角色/删管理员/管理注册密令等全部权限）。密令仅以 bcrypt 哈希存入数据库，首次初始化写入，
> 之后修改 `.env` 不再影响线上，变更密令请到「管理后台 → 系统设置」。
