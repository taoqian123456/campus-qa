...
用法：
  ...
  venv\\Scripts\\python.exe perf_test.py --provider zhipu   # 换模型测耗时（多模型对比）
...
def main():
    ...
    provider_note = f"，provider={args.provider}" if args.provider else ""
    print(f"性能测试：{len(questions)} 个问题{provider_note}")
    print("=" * 70)
    ...
    for i, q in enumerate(questions, 1):
        ...
        try:
            result = answer_question(q, provider=args.provider)
    ...
    # 默认文件名带 provider 后缀：多模型耗时对比并排存放不互相覆盖
    out_path = Path(args.output) if args.output else BASE_DIR / (
        f"perf_results_{args.provider}.csv" if args.provider else "perf_results.csv"
    )
